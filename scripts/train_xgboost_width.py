...
# Outputs
WIDTH_DIR = BASE_DIR / "result" / "width"
WIDTH_DIR.mkdir(parents=True, exist_ok=True)

OUT_MODEL = WIDTH_DIR / "xgb_width_model_bundle.pkl"
OUT_METRICS = WIDTH_DIR / "xgb_width_metrics.txt"
OUT_PRED = WIDTH_DIR / "xgb_width_predictions.csv"
OUT_SCATTER = WIDTH_DIR / "true_vs_pred_width.png"
OUT_HIST = WIDTH_DIR / "abs_error_hist_width.png"
...
def main():
    print("\n--- TRAINING XGBOOST CRACK WIDTH REGRESSOR ---\n")
    ...
    y = grouped["crack_width"].to_numpy(float)
    ...
    acc_01 = np.mean(abs_err <= 0.10) * 100
    acc_025 = np.mean(abs_err <= 0.25) * 100
    acc_05 = np.mean(abs_err <= 0.50) * 100

    print(f"MAE  : {mae:.4f} mm")
    print(f"RMSE : {rmse:.4f} mm")
    print(f"R²   : {r2:.4f}")
    print(f"±0.10mm accuracy : {acc_01:.2f}%")
    print(f"±0.25mm accuracy : {acc_025:.2f}%")
    print(f"±0.50mm accuracy : {acc_05:.2f}%")

    with open(OUT_METRICS, "w") as f:
        f.write(f"MAE_mm={mae:.6f}\n")
        f.write(f"RMSE_mm={rmse:.6f}\n")
        f.write(f"R2={r2:.6f}\n")
        f.write(f"Acc_0.10mm={acc_01:.6f}\n")
        f.write(f"Acc_0.25mm={acc_025:.6f}\n")
        f.write(f"Acc_0.50mm={acc_05:.6f}\n")

    # 6. SAVE PREDICTIONS
    pd.DataFrame({
        "y_true_width_mm": y_test,
        "y_pred_width_mm": y_pred,
